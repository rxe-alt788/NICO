...
def load(beachwatch_path: str | None, beachwatch_url: str | None, imos_path: str | None, imos_url: str | None, beach_ids: List[str]) -> tuple[Dict[str, Dict[Any, Dict[str, Any]]], Dict[str, Any]]:
    ...
    if text:
        ...
        for row in rows:
            ...
            out[bid].setdefault(ts, {}).update({
                ...
                'enterococci': safe_float(first(row, 'enterococci'))
            })
        status['beachwatch'] = {'state': 'LOADED', 'source': source, 'rows': len(rows), 'note': 'Beachwatch historical exports commonly include enterococci, water temperature and conductivity; turbidity is used only when explicitly present.'}
    else:
    ...
    if text:
        rows = csv_rows(text); parsed = []; sst: Dict[str, List[float]] = defaultdict(list)
        for row in rows:
            ...
            value = safe_float(first(row, 'sst_c', 'sst', 'sea_surface_temperature'))
            ...
            parsed.append((bid, ts, value, up, acoustic))
            if value is not None: sst[bid].append(value)
        for bid, ts, value, up, acoustic in parsed:
            ...
            out[bid].setdefault(ts, {}).update({'sstC': value, 'sstAnomaly': anomaly, 'upwellingIndex': up, 'seasonalAcousticDensityPct': acoustic})
        status['imos'] = {'state': 'LOADED', 'source': source, 'rows': len(rows), 'note': 'Pilot SST anomaly uses supplied-window median pending climatological baseline.'}
    else:
